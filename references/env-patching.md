...
- 常见项：`navigator`、`webdriver`、`crypto`、`atob/btoa`、`TextEncoder`、`cookie`、`location`、`module/exports`。
- 避免一次性全局模拟浏览器。
...
- 注册表已含忠实 `document.cookie`（带 cache 的 get/set）与 `location.href`（set 后分量联动）实现，以及 `module/exports` 中和（使 `typeof` 为 `undefined`，强制 UMD/CJS 代码走浏览器分支）。
