...
4. 若任务已通过 `env-pass`，或目标明确是“补环境后提纯算法”，继续读 `pure-extraction.md`
...
3. 最后按模板新建流程：
   - `parameter-methodology-template.md`
   - `parameter-site-mapping-template.md`
...
如果目标参数需要跨请求、运行时栈、异步链路或浏览器环境信号关联，使用 `collect_code` 开启 `enableRuntimeSampler`、`enableAdvancedDynamicAnalysis` 和 `targetParams`。完整协议见 `dynamic-analysis.md`。
...
1. `new_page`
2. 先读 `reverse-bootstrap.md`，再按其要求继续读 `case-safety-policy.md` 与 `reverse-workflow.md`
3. `analyze_target`
...
完整参数与字段说明请查：`tool-reference.md`
动态分析能力说明请查：`dynamic-analysis.md`
真实 MCP 验收矩阵请查：`real-validation.md`

## 12) 参数复现模板复用（推荐）

遇到“某个参数可复现”任务时，先走模板而不是临时写脚本：

1. 先读 `reverse-bootstrap.md`
2. 再填站点无关模板：`parameter-methodology-template.md`
3. 再填站点映射模板：`parameter-site-mapping-template.md`
