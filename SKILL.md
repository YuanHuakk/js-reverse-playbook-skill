...
- Hook 不足时再考虑断点
- 参数生成链路跨请求、异步、worker、WASM、反调试或指纹依赖时，使用 `collect_code` 启用 `enableRuntimeSampler` / `enableAdvancedDynamicAnalysis` / `targetParams`
- 明文 cookie、token、请求体证据只能在授权任务中显式启用 `includeSensitiveEvidence` 后输出，并且必须限制长度与记录用途
...
- 动态分析能力按 `references/project-docs/dynamic-analysis.md`
- 真实浏览器 / MCP 链路验收按 `references/project-docs/real-validation.md`
- `references/cases/*` 只允许抽象 case（映射/判定口径）
- 站点级可复用实操流程不随本 skill 分发，维护在配套项目仓库（私有）
...
- 任务产物脚手架（可直接复制到 `artifacts/tasks/<task-id>/`）：`references/task-template/`
...
- 动态分析：`references/project-docs/dynamic-analysis.md`
- 真实验收：`references/project-docs/real-validation.md`
...
- 更新提示词：`references/project-docs/reverse-update-prompt-template.md`
- 报告模板：`references/project-docs/reverse-report-template.md`
- 算法升级 / first divergence：`references/project-docs/algorithm-upgrade-template.md`
